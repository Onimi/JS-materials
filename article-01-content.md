...
2. 1. в случае совпадения значений возвращается `true`,
2. 2. в случае различных значений возвращается `false`.
3. Если `left` и `right` - являются числами, тогда:
3. 1. Если они оба не являются `NaN` и их значения совпадают, то возвращается `true`,
3. 2. Если одно из них `0`, а другое `-0`, то возвращается `true`,
3. 3. Во всех остальных случаях возвращается `false`.
